...
import { CAR_CLASS_RENTAL } from "../../GraphQL/rentFeatures";
...
import {UPLOAD_CAR_CLASS_RENT}  from '../../GraphQL/rentMutations';
import {DELETE_CAR_CLASS_RENT} from '../../GraphQL/deleteRentMutations'
...
function Class() {
  ...
  const queryMultiple = () => {
    ...
    const classsale = useQuery(CAR_CLASS_RENTAL);
    return [classsale];
  }
  ...
  useEffect(() => {
    ...
    if(data5){
      setclass_rental(data5.carClassRentels);
    }
  ...
  const [createCarClass,{ data}] = useMutation(UPLOAD_CAR_CLASS_RENT,{
    onCompleted: (data) => {
  ...
  const [deleteCarClass] = useMutation(DELETE_CAR_CLASS_RENT,{
    onCompleted: (data) => {
     setSucess(true)
    }
  });
  ...
  <div>
       ...
       <ImageUpload ImageID={setImageId}/>
               ...
               <Table columns={columns} size="middle" dataSource={class_rental} />
