...
import { CAR_VENDOR_SALES } from "../../GraphQL/saleFeatures";
...
import {UPLOAD_CAR_VENDOR_SALE}  from '../../GraphQL/saleMutations';

import {DeleteTwoTone} from '@ant-design/icons';
import {DELETE_CAR_VENDOR_SALE}  from '../../GraphQL/DeleteSaleMutations';
...
function Vendor() {
  ...
  const queryMultiple = () => {
    ...
    const vendorsale = useQuery(CAR_VENDOR_SALES);
    return [vendorsale];
  }
  ...
  useEffect(() => {
    if(data7){
      setVendor_sale(data7.carVendors);
    }
  ...
  const [createCarVendor,{ data}] = useMutation(UPLOAD_CAR_VENDOR_SALE,{
    onCompleted: (data) => {
      handleCancel()
     setSucess(true)
  ...
  const [deleteCarVendor] = useMutation(DELETE_CAR_VENDOR_SALE,{
    onCompleted: (data) => {
     setSucess(true)
     console.log(data)
    }
  ...
  <div>
       ...
       <ImageUpload ImageID={setImageId}/>
               ...
               <Table columns={columns} size="middle" dataSource={vendor_sale} />
